...
1. ``ATM_STORAGE_BACKEND`` env var (overrides config).
...
B. Third-party plugins registered under the ``atm.storage_backends`` entry-
   point group.
...
without knowing each backend's constructor shape::
    ...
    [project.entry-points."atm.storage_backends"]
...
from atm.core.storage.base import StorageBackend

logger = logging.getLogger(__name__)

_ENTRY_POINT_GROUP = "atm.storage_backends"
...
def get_storage_backend() -> StorageBackend:
    ...
    with _lock:
        ...
        config = get_settings()
        backend_name = (
            os.environ.get("ATM_STORAGE_BACKEND")
