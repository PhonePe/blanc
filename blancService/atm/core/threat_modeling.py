...
# Database Imports
from atm.db.database import SessionLocal, engine 
...
class ThreatFrameworkConfig:
    ...
    system_role: str
    skill_name: str = "threat_analysis"  # skill file to load from atm/skills/definitions/
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"Failed category {category} for {framework.name}: {e}")
...
def threat_modeling_pipeline(assessment_id: str, framework_name: str = "STRIDE") -> dict:
    ...
    try:
        ...
        db.close()
        logger.error(f"[{assessment_id}] Failed to transition to PROCESSING: {e}")
        raise
    ...
    try:
        ...
        db2 = SessionLocal()
        try:
            transition_assessment(db2, assessment_id, AssessmentState.FAILED, AssessmentStage.THREAT_MODELING, error_message=str(e))
        finally:
            db2.close()
        raise
    ...
    try:
        ...
        transition_assessment(db3, assessment_id, AssessmentState.REVIEW, AssessmentStage.THREAT_MODELING)
        ...
        transition_assessment(db3, assessment_id, AssessmentState.FAILED, AssessmentStage.THREAT_MODELING, error_message=str(e))
        raise
    finally:
        db3.close()
