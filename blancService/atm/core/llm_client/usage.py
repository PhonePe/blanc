...
class LoggingUsageSink(UsageSink):
    ...
    def __init__(self, logger: Optional[logging.Logger] = None, level: int = logging.INFO):
        self._logger = logger or logging.getLogger("atm.llm.usage")
        self._level = level
...
class SqlAlchemyUsageSink(UsageSink):
    ...
    def __init__(self):
        self._logger = logging.getLogger("atm.llm.usage")
    ...
    def record(self, usage: UsageRecord) -> None:
        ...
        try:
            ...
            from atm.db_models.models import LLMUsage as LLMUsageRow
