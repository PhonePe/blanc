...
* :func:`get_provider` — provider factory that supports built-in names
  ...
  the ``atm.llm_providers`` entry-point group.

Plugging in a new provider from another package::

    # myorg/atm_anthropic/__init__.py
    from atm.core.llm_client import LLMProvider

    class AnthropicProvider(LLMProvider):
        ...

    # myorg/pyproject.toml
    # [project.entry-points."atm.llm_providers"]
    # anthropic = "myorg.atm_anthropic:AnthropicProvider"

Then set ``ATM_LLM_PROVIDER=anthropic`` (or override
...
_ENTRY_POINT_GROUP = "atm.llm_providers"
...
def _build_default_client() -> LLMClient:
    ...
    provider_name = (
        os.environ.get("ATM_LLM_PROVIDER")
...
def _build_default_token_provider(config, provider_name: str) -> Optional[TokenProvider]:
    ...
    if os.environ.get("ATM_LLM_API_KEY"):
        return EnvTokenProvider("ATM_LLM_API_KEY")
    ...
    if provider_name == "openai":
        raise RuntimeError(
            ...
            "ATM_LLM_API_KEY, or openaiconfig.api_key."
        )
    return None
