...
1. Set ``ATM_SKILLS_DIRS`` to a ``os.pathsep``-separated list of directories.
2. Or register an entry point in the ``atm.skills`` group that points to a
   module exposing a ``skills_dir: str`` (or ``Path``) attribute, or a
   callable returning a directory path.

Resolution order (first match wins): ``ATM_SKILLS_DIRS`` directories,
...
def _skill_dirs() -> Tuple[Path, ...]:
    ...
    Order: ``ATM_SKILLS_DIRS`` paths, entry-point paths, built-in dir.
    Missing directories are dropped with a debug log.
    """
    dirs: List[Path] = []
    seen: set = set()

    env_value = os.environ.get("ATM_SKILLS_DIRS", "").strip()
    if env_value:
...
def _entry_point_dirs() -> List[Path]:
    """Discover skill directories contributed via the ``atm.skills`` entry point group."""
    try:
