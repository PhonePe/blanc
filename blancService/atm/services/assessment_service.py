...
from atm.crud import assessment_crud
from atm.core.state_machine import transition_assessment, transition_image, get_image_states
from atm.config_parsers.settings import get_settings
from atm.util.file_sniff import safe_filename
    ...
    async def _create_new_assessment_inner(
        ...
        for image_file in images:
            ...
            try:
                ...
                logging.error(f"File save failed for {safe_filename(image_file.filename)}: {str(e)}")
        ...
        if supporting_docs:
            for doc_file in supporting_docs:
                ...
                try:
                    ...
                    logging.error(f"Supporting doc save failed for {safe_filename(doc_file.filename)}: {str(e)}")
