...
def ensure_database_exists(url: str | None = None) -> None:
    ...
    ``Unknown database 'blanc'``. This helper connects with the schema
    name stripped from the URL, runs ``CREATE DATABASE IF NOT EXISTS``,
    and returns. Idempotent — safe to run on every startup.

    Call this from the app factory (:func:`blanc.app.create_app`),
    NOT at module import — otherwise CLI tools / tests that only need
    the models can't import ``blanc.db.database`` without hitting the
    live server.
    """
    if url is None:
