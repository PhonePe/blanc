...
from blanc.core.document_analysis import (
    analyze_single_image_phase_a,
    analyze_single_image_phase_a_from_mermaid,
    analyze_single_image_phase_b,
)
from blanc.core.llm_client import set_assessment_context
from blanc.core.threat_modeling import threat_modeling_pipeline
from blanc.db.database import get_db_session
from blanc.db_models.models import Assessment, DocumentAnalysis, AssessmentState
...
async def _handle_image_phase_a(rmq_msg: RMQMessage):
    ...
    try:
        ...
        # The context manager closes the session before the LLM call —
        # we don't want the DB connection held for the several-second
        # vision round-trip. `analyze_single_image_phase_a` opens its
        # own session.
...
async def _handle_image_phase_a_from_mermaid(rmq_msg: RMQMessage):
    ...
    try:
        ...
        logger.exception(f"[{assessment_id}][img:{image_id}] Phase A (from mermaid) failed")
...
async def _handle_image_phase_b(rmq_msg: RMQMessage):
    ...
    try:
        ...
        logger.exception(f"[{assessment_id}][img:{image_id}] Phase B failed")
...
async def _handle_pdf_ingestion(rmq_msg: RMQMessage):
    ...
    try:
        ...
        logger.exception(f"[{assessment_id}] Rejecting PDF_INGESTION")
        return
    ...
    try:
        ...
        logger.exception(f"[{assessment_id}] PDF ingestion failed for '{filename}'")
...
async def _handle_threat_modeling(rmq_msg: RMQMessage):
    ...
    try:
        ...
        logger.exception(f"[{assessment_id}] Threat modeling pipeline failed")
