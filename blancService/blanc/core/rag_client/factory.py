...
1. ``BLANC_RAG_BACKEND`` env var (overrides config).
...
B. Third-party plugins registered under the ``blanc.rag_backends`` entry-point
   group.
...
knowing each backend's constructor shape::
    ...
    [project.entry-points."blanc.rag_backends"]
...
:class:`~blanc.core.rag_client.vector_db.VectorDBClient`:
...
_ENTRY_POINT_GROUP = "blanc.rag_backends"
...
def _build_local(config):
    from blanc.core.rag_client.local_vector_db import LocalVectorDB
...
def _build_http(config):
    from blanc.core.rag_client.vector_db import VectorDBClient
...
def get_rag_client(config=None):
    ...
    with _lock:
        ...
        backend_name = (
            os.environ.get("BLANC_RAG_BACKEND")
