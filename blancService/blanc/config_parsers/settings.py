...
Resolution order (highest priority first):
    ...
    1. ``BLANC_CONFIG_PATH`` env var  → absolute path to any YAML file.
    2. ``ENV`` env var              → picks ``blanc/config/<env>.yml``
...
from blanc.config_parsers.config_models import AppConfig
...
def _resolve_config_path() -> Path:
    ...
    override = os.environ.get("BLANC_CONFIG_PATH", "").strip()
    if override:
        path = Path(override).expanduser().resolve()
        if not path.is_file():
            _die(f"BLANC_CONFIG_PATH points at a missing file: {path}")
        return path
    ...
    if not path.is_file():
        _die(
            ...
            f"  Either set BLANC_CONFIG_PATH, ENV, or drop a {env_name}.yml "
            f"under {_CONFIG_DIR}."
        )
    return path
